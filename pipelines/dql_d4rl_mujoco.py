    ...
    if args.mode == "train":
        ...
        for batch in loop_dataloader(dataloader):
            ...
            if (n_gradient_step + 1) % args.log_interval == 0:
                ...
                reward_mean, reward_std = eval_policy(actor, dataset, args, env)
...
def eval_policy(agent, dataset, args, env):
    agent.eval()
    env_eval = gym.vector.make(args.task.env_name, args.num_envs)
    normalizer = dataset.get_normalizer()
    episode_rewards = []
    ...
    for i in range(args.num_episodes):
        ...
        while not np.all(cum_done) and t < 1000 + 1:
            ...
            action, log = agent.sample(
                ...
                sample_steps=args.sampling_steps,
                condition_cfg=obs,
                use_ema=args.use_ema, w_cfg=1.0, temperature=args.temperature)
            action = action.reshape(args.num_envs, act_dim)
            act = action
