    ...
    def ddim_step_with_logprob(
        ...
        for i in reversed(loop_steps):
            ...
            if i > 1:
                ...
                log_prob = (
                        -((xt.detach() - xt_mean) ** 2) / (2 * (stds[i] ** 2))
                        - torch.log(stds[i])
        ...
        if self.classifier is not None and w_cg != 0.:
            with torch.no_grad():
                t = torch.zeros((n_samples,), dtype=torch.long, device=self.device)
                logp = self.classifier.logp(xt, t, condition_vec_cg)
        if not torch.equal(xt, all_xt[-1]):
            raise ValueError(
                f"new_act ({xt.shape})与all_xt[-1] ({all_xt[-1].shape})不匹配\n"
            )
        if self.clip_pred:
            xt = xt.clip(self.x_min, self.x_max)

        return xt, all_xt, all_log_probs
...
class ContinuousDiffusionSDE(BaseDiffusionSDE):
    ...
    def sample(
        ...
        # ===================== Initialization =====================
        log = {
            "sample_history": torch.empty((n_samples, sample_steps + 1, *prior.shape)) if preserve_history else None,
            'eps_theta_history': torch.empty((n_samples, sample_steps + 1, *prior.shape)) if preserve_history else None,
            't_history': torch.empty((sample_steps + 1, n_samples)) if preserve_history else None
        }
        ...
        for i in reversed(loop_steps):
            ...
            xt = xt * (1. - self.fix_mask) + prior * self.fix_mask
            if preserve_history:
                log["sample_history"][:, sample_steps - i + 1] = xt
                log['eps_theta_history'][:, sample_steps - i] = eps_theta
                log['t_history'][sample_steps - i] = t
    ...
    def pre_history(
        ...
        model = self.model if not use_ema else self.model_ema
        xt, t, _ = self.add_noise(x0)

        condition = model["condition"](condition) if condition is not None else None

        eps_theta = model["diffusion"](xt, t, condition)
