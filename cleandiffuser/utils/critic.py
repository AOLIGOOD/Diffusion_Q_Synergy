...
class DMA(nn.Module):
    ...
    def __init__(self, obs_dim: int, act_dim: int, discount: float = 0.99, hidden_dim: int = 256, ):
        super().__init__()
        ...
        self.V_targ = deepcopy(self.V).requires_grad_(False).eval()
    ...
    def update_Q(self, obs, act, rew, obs_next, done):
        with torch.no_grad():
            td_target = rew + self.discount * (1 - done) * self.V_targ(obs_next)
